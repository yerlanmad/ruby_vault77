class App
  ...
  def create_route_menu
    ...
    when (1..stations.size)
      ...
      puts stations_to_add.map(&:name).join(' | ')
      create_route_menu
    when 99
      routes_menu unless stations_to_add.size > 1
      routes << Route.new(stations_to_add.first, stations_to_add.last)
      routes_menu
    when 0
      routes_menu
    else
  ...
  def move_train_menu
    ...
    when (1..trains.size)
      ...
      puts "Previous station: #{trains[menu_item - 1].previous_station.name}\n" +
           "Current station: #{trains[menu_item - 1].current_station.name}\n" +
           "Next station: #{trains[menu_item - 1].next_station.name}"
      move_train_menu
    when 0
      trains_menu
    else
  ...
  def choose_station_menu
    stations.map(&:name).each_with_index { |st, i| puts "| #{i + 1} | #{st}" }
    print "| 0 | Back\nChoose station: "
  end
